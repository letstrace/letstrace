// Point Clustering
// Based on https://github.com/yetzt/node-geocluster
// Public domain / unlicensed: http://unlicense.org/UNLICENSE
...
function PointClustering(elements, bias) {
  if (!(this instanceof PointClustering))
    return new PointClustering(elements, bias);
  return this._cluster(elements, bias);
}

// geodetic distance approximation
PointClustering.prototype._dist = function(lat1, lon1, lat2, lon2) {
...
PointClustering.prototype._centroid = function(set) {
  return set
    .reduce(
      function(s, e) {
...
PointClustering.prototype._clean = function(data) {
  return data.map(function(cluster) {
    return [cluster.centroid, cluster.elements];
  });
};

PointClustering.prototype._cluster = function(elements, bias) {
  var self = this;
